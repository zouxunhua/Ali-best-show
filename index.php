<?php
  ...
  // 准备sql
  $sql = "select * from posts where status ='published' order by created desc limit 5";
  // 执行sql
  $data = my_SELECT($sql);
  // 渲染页面
  // var_dump($data);
  ...
  // var_dump($slideDataJSON);
  // json_encode php的数组 对象->json格式的字符串
  // 参数2 传入true 会格式化为关系型数组
  $slideData = json_decode($slideDataJSON,true);
  // var_dump($slideData);
  // ->php中对象 点语法
  // var_dump( $slideData[0]->image);
  ...
  <div class="wrapper">
    ...
    <div class="content">
      <div class="swipe">
        <ul class="swipe-wrapper">
          <?php for($i=0;$i<count($slideData);$i++): ?>
            <li>
              <a href="<?php echo $slideData[$i]['link']; ?>">
                <img style="width:830px;height:310px;" src="<?php echo $slideData[$i]['image']; ?>">
                <span><?php echo $slideData[$i]['text']; ?></span>
              </a>
            </li>
        <?php endfor; ?>
        </ul>
      ...
      <div class="panel focus">
        ...
        <ul>
          <li class="large">
            <a href="./detail.php?id=<?php echo $data[0][0]; ?>">
              <img src="<?php echo $data[0][3]; ?>" alt="">
              <span><?php echo $data[0][2]; ?></span>
            </a>
          </li>
        <?php for($i=1;$i<count($data);$i++): ?>
          <li>
            <a href="./detail.php?id=<?php echo $data[$i][0]; ?>">
              <img src="<?php echo $data[$i][3]; ?>" alt="">
